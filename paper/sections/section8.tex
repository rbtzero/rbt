...
\]
$\gamma_E$ being Euler's constant.

\paragraph{Epoch 60 freeze and network stability.}
At scale factor \(a=5.8\times10^{-59}\)---exactly sixty causal ticks after the
\(\delta\)-glitch---every open tensor loop longer than one Planck cell acquires
even parity and becomes irreversible.  The ledger therefore locks in
\(2^{60}\) frozen micro-histories, fixing the primordial curvature spectrum
and guaranteeing that large-scale homogeneity persists.  All later
acoustic features, including the DESI 3 h\(^{-1}\)Mpc BAO scale, trace directly
to this Epoch-60 freeze.\label{sec:epoch60}

Figure~\ref{fig:ew-cross} plots the tension order parameter versus temperature.
